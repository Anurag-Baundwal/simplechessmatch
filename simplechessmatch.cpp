...
int parse_cmd_line_options(int argc, char* argv[])
{
   try
   {
      ...
      desc.add_options()
         ...
         ("depth",      po::value<uint>(&options.tc_depth)->default_value(0), "fixed search depth per move (0 to disable).")
         ("nodes",      po::value<uint64_t>(&options.tc_nodes)->default_value(0), "fixed maximum nodes per move (0 to disable).")
